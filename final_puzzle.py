global g
g=0   #no of moves made
def print_board(elements):
    for i in range(9):
        if i%3 == 0:         #when to start new row
            print()         #newline
        if elements[i]==-1:     
            print("_", end = " ")
        else:
...
def solvable(start):
    inv=0           # no of variation created

    for i in range(9):      
        if start[i] <= 1:         # tile with empty space or value 0         
            continue                    
        for j in range(i+1,9):      
            if start[j]==-1:          #if rightside tile is an empty space      
                continue
            if start[i]>start[j]:               
                inv+=1
    if inv%2==0:                    #if no of inversion even then solvable else not
        return True
    return False
